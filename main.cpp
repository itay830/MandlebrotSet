...
public:
	...
	long double scaleRange(long double old_value, long double old_max, long double old_min, long double new_max,
	                       long double new_min) {
	...
	void Draw() {
		...
		// DrawTextureRec(rTexture.texture, Rectangle{0, 0, static_cast<float>(rTexture.texture.width), -static_cast<float>(rTexture.texture.height), Vector2{0, 0}, WHITE);
		DrawTextureRec(rTexture.texture, Rectangle{
			               0, 0, static_cast<float>(rTexture.texture.width),
			               -static_cast<float>(rTexture.texture.height)
		               }, Vector2{0, 0}, WHITE);
		if (isZoomSelected) {
			DrawRectangleLines(zoomRec.x, zoomRec.y, zoomRec.width, zoomRec.height, WHITE);
			// DrawText(zoomRec.x, );
		}
		DrawText("Drag your mouse to make rectangles around places you're interested in!", 10, 10, 40, WHITE);
		DrawText("PRESS `R` for RESET view!", 10, 50, 40, WHITE);
		EndDrawing();
	}
	...
	bool Controls() {
		...
		if (IsMouseButtonDown(MOUSE_BUTTON_LEFT)) {
			...
			zoomRatio = Rect{
				...
				scaleRange(zoomRec.y, SH, 0, zoomRatioTemp.y, zoomRatioTemp.height)
			}; // zoomRec.y + zoomRec.height
...
void threadDraw(Game &game, int numThread) {
	...
	thread threads[8];
	for (int i = 1; i <= 8; i++) {
		threads[i - 1] = std::thread(&Game::setColorMap, &game, 0, game.SW, game.SH * (i - 1) / 8, game.SH * i / 8);
	}
...
int main() {
	int WIDTH = 0;
	int HEIGHT = 0;
	unsigned int maxIter = 1500;

	InitWindow(0, 0, "Mandelbrotset");
	ToggleFullscreen();
	SetTargetFPS(60);
